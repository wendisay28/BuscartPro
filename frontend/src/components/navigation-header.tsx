...
export default function NavigationHeader({ hideSearch = false }: NavigationHeaderProps) {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <nav className="bg-white dark:bg-gray-900/80 dark:backdrop-blur-md border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 hidden md:block shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Left Side: Logo and Search */}
            <div className="flex items-center space-x-4">
              <Link href="/" className="flex-shrink-0 flex items-center space-x-2">
                <div className="w-8 h-8 bg-[#bb00aa] rounded-lg flex items-center justify-center">
                  <Palette className="text-white h-4 w-4" />
                </div>
                <span className="font-heading font-bold text-lg text-gray-800 dark:text-white">
                  Busc<span className="text-[#bb00aa]">Art</span>
                </span>
              </Link>
              {!hideSearch && (
                <div className="relative hidden sm:block">
                  <LocationSelector 
                    onLocationSelect={(location) => console.log('Ubicación seleccionada:', location)}
                    variant="header"
                    className="w-64"
                  />
                </div>
                ...
                {navigationItems.map((item) => (
                  <Link
                    ...
                    className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      isActive(item.href)
                        ? "text-white bg-[#bb00aa] hover:bg-[#a00090]"
                        : "text-gray-600 dark:text-gray-300 hover:bg-[#bb00aa]/10 hover:text-[#bb00aa] dark:hover:bg-[#bb00aa]/20"
                    }`}
                  >
                  ...
                  <>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="relative hover:bg-[#bb00aa]/10 text-gray-600 hover:text-[#bb00aa] dark:text-gray-300 dark:hover:text-[#bb00aa]">
                          <Bell className="h-5 w-5" />
                          {notificationCount > 0 && (
                            <Badge className="absolute -top-1 -right-1 h-4 w-4 justify-center p-1 text-xs bg-[#bb00aa] hover:bg-[#a00090]">
                              {notificationCount}
                            </Badge>
                          )}
                        </Button>
                    ...
                    {/* Messages */}
                    <Button variant="ghost" size="icon" className="hover:bg-[#bb00aa]/10 text-gray-600 hover:text-[#bb00aa] dark:text-gray-300 dark:hover:text-[#bb00aa]" asChild>
                      <Link href="/messages">
